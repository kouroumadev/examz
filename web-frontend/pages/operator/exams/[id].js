  ...
  return (
    <div className="mt-12">
      ...
      {listSection.sections.map((itemSection, index) => (
        <ListSession key={index} type="exams" idExamPractice={id} index={index} itemSection={itemSection} onOpenDeleteModal={onOpenDeleteModal} setQuestionSelectedId={(data) => setQuestionSelectedId(data)} setSelectedData={(data) => setSelectedData(data)} setSelectedName={(data) => setSelectedName(data)} onOpen={onOpen} role="operator"/>
      ))}
