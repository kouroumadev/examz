  ...
  return (
    <div className="mt-12">
      ...
      {listSection.sections.map((itemSection, index) => (
        <ListSession key={index} type="exams" idExamPractice={id} index={index} itemSection={itemSection} onOpenDeleteModal={onOpenDeleteModal} setQuestionSelectedId={(data) => setQuestionSelectedId(data)} setSelectedData={(data) => setSelectedData(data)} setSelectedName={(data) => setSelectedName(data)} onOpen={onOpen} role="institute"/>
      ))}
        ...
        <ModalContent>
          ...
          <ModalBody>
            ...
            <div className="flex flex-row-reverse gap-4 mt-4" >
              <Link href={`/institute/exams/section/${listSection.id}_id=${selectedData}#${questionType}`}>
