      ...
      await apiExamCategoryType.allConfig()
        .then((res) => {             
          res.data.data.map((item) => {
             if(examCatId == item.exam_category_id) {
              item.data.sections.map((section) => {
                if(sectionName == section.name) {
                   console.log('je suis la')
                  //  console.log('la valeur est ' + isRequired)
                  //  console.log(section.name)
                   setValue("mark", section.questions.mark)
                  setNegativeMark(section.questions.negative_mark)
                  setMark(section.questions.mark)
                  // console.log(section.questions.negative_mark)
                }
              })
             }
              ...
              <div className="bg-blue-6 p-4 text-sm" key={indexQuestion}>
                ...
                {itemQuestion.type === "simple" && (
                  <div className="flex justify-between mt-2 bg-white p-4">
                    <div className="text-sm font-bold">{firstNumber + indexQuestion}. {itemQuestion.type === 'simple' ? 'Simple' : 'Paragraph'} Question</div>
                  </div>
                )}
                {itemQuestion.type === 'paragraph' && (
                  <>
                    <div className="flex justify-between mt-2">
                      <div className="text-1xl font-bold">{firstNumber + indexQuestion}. {itemQuestion.type === 'simple' ? 'Simple' : 'Paragraph'} Question</div>
                    </div>
                    <div className="flex flex-col md:flex-row mt-4 gap-4">
                      ...
                      <div className="w-full">
                          <p>Compulsory</p>
                          <Select bg='white' {...register(`questions[${indexQuestion}].is_required`)} size="sm" variant='outline' iconColor="blue">
                    ...
                    <div className="mt-4">
                      ...
                      <div className="w-full  bg-white rounded-lg " style={{ lineHeight: 2 }} >
                        ...
                        <Quill type={s3.QUESTION_ANSWER} data={itemQuestion.instruction} register={(data) => setDataForm(`questions[${indexQuestion}].instruction`, data)} />
                      </div>
                    </div>
                    <div className="mt-4">
                      ...
                      <div className="w-full  bg-white rounded-lg " style={{ lineHeight: 2 }} >
                        ...
                        <Quill type={s3.QUESTION_ANSWER} data={itemQuestion.instruction} register={(data) => setDataForm(`questions[${indexQuestion}].paragraph`, data)} />
                      </div>
                    </div>
                  </>
                )}
                  ...
                  return (
                    <div className={`bg-white p-4 ${itemQuestion.type === "paragraph" && 'mt-8'}`} key={indexEachQuestion}>
                      ...
                      <div className="flex flex-col md:flex-row mt-4 gap-4">
                        ...
                        <div className="w-full">
                          <p>Compulsory</p>
                          <Select bg='white' {...register(`questions[${indexQuestion}].question_items[${indexEachQuestion}].is_required`)} size="sm" variant='outline' iconColor="blue">
                            <option value="1">Compulsory</option>
                            <option value="0">Not Compulsory</option>                    
                          </Select>
                        </div>
                      ...
                      <div className="mt-4">
                        ...
                        <div className="w-full  bg-white rounded-lg " style={{ lineHeight: 2 }} >
                          ...
                          <Quill type={s3.QUESTION_ANSWER} data={itemQuestion.instruction} register={(data) => setDataForm(`questions[${indexQuestion}].question_items[${indexEachQuestion}].question`, data)} />
                        </div>
                      </div>
                      <div className="mt-4">
                        ...
                        <Select bg='white' onClick={(e) => {
                          ...
                          <option value="numeric">Numeric Answer Type</option>
                        </Select>
                        {errors && (
                          ...
                          return (
                            <div className={`${itemAnswer.correct === 1 ? 'bg-blue-6 border-blue-1' : 'bg-white'} my-2  p-4 border rounded-lg`} key={indexAnswer}>
                              ...
                              <div className='flex gap-2'>
                                ...
                                { eachQuestion.answer_type === 'multiple' && (
                                  // if multiple answer
                                  <div className="flex cursor-pointer" onClick={() => {
                                    const temp = questions
                                    temp.map((itemQ) => {
                                ...
                                {/* new code */}
                                { eachQuestion.answer_type === 'numeric' && (
                                  
                                  <div className="flex cursor-pointer" onClick={() => {
                                    const temp = questions
                                   
                                    temp.question_items.options.correct = 1
                                    setValue(`questions[${indexQuestion}].question_items[${indexEachQuestion}].options[${indexAnswer}].correct`, 1)
                                    setQuestions([...temp])
                                  }}>
                                    {
                                ...
                                {/* from sonu */}
                                {/* <div className="w-full  bg-white rounded-lg " style={{lineHeight: 2}}>
                                  <Quill type={s3.QUESTION_ANSWER} subType={s3.ANSWER} data={itemAnswer.title} register={(data) => setDataForm(`question_items[${indexEachQuestion}].options[${indexAnswer}].title`, data)}/>
                                </div> */}
                        ...
                        <div className="mt-4">
                          ...
                          <div className="w-full  bg-white rounded-lg " style={{ lineHeight: 2 }} >
                            ...
                            <Quill type={s3.QUESTION_ANSWER} data={itemQuestion.instruction} register={(data) => setDataForm(`questions[${indexQuestion}].question_items[${indexEachQuestion}].answer_explanation`, data)} />
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row mt-4 gap-4 mb-4">
                          <div className="w-full">
                            ...
                            <input type="number" value={mark} className=" w-full form border rounded p-2" placeholder="0" {...register(`questions[${indexQuestion}].question_items[${indexEachQuestion}].mark`)} />
                            {setValue(`questions[${indexQuestion}].question_items[${indexEachQuestion}].mark`, mark)}
                          </div>
                          <div className="w-full">
                            ...
                            <input type="number" value={negativeMark} className="w-full form border rounded p-2" placeholder="0" {...register(`questions[${indexQuestion}].question_items[${indexEachQuestion}].negative_mark`)} />
                            {setValue(`questions[${indexQuestion}].question_items[${indexEachQuestion}].negative_mark`, negativeMark)}
                          </div>
                        </div>
                      </div>
                    </div>
