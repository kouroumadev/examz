...
import { Marking } from "../Question/question";
...
export default function CreateQuiz({ role }) {
  ...
  const submitQuiz = async (req) => {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      data.append(`${field}[level]`, questions[i].level)
      data.append(`${field}[tag]`, questions[i].tag)
      data.append(`${field}[mark]`, questions[i].mark)
      data.append(`${field}[answer_type]`, questions[i].answer_type)
      data.append(`${field}[negative_mark]`, questions[i].negative_mark)
      data.append(`${field}[question]`, questions[i].question)
      data.append(`${field}[answer_explanation]`, questions[i].answer_explanation)
  ...
  const onAnswerExplanation = (index, field) => {
    const temp = questions
    temp.map((item, i) => {
      if (i === index)
        item.answer_explanation = field
    })
    console.log(temp)
    setQuestions([...temp])
  }

  const onMark = (index, field) => {
    const temp = questions
    temp.map((item, i) => {
      if (i === index)
        item.mark = field
  ...
  const onNegativeMark = (index, field) => {
    const temp = questions
    temp.map((item, i) => {
      if (i === index)
        item.negative_mark = field
    })
    setQuestions([...temp])
  }

  useEffect(() => console.log(questions), [renderQuestion])
                  ...
                  return (
                    <div className={`bg-white p-4 mt-8`} key={indexEachQuestion}>
                      <div className="flex justify-between">
                        <div className="flex font-bold text-xl">Question {indexEachQuestion + 1}</div>
                        {questions.length > 1 && (
                          <div className="flex my-auto text-red-1 border border-red-1 p-2 rounded cursor-pointer"
                            onClick={() => {
                              const temp = questions
                              const newData = []
                              temp.map((item) => {
                              ...
                              setRenderQuestion(!renderQuestion)
                            }}
                          >Delete Question</div>
                        )}
                      </div>
                      <div className="flex flex-col md:flex-row mt-4 gap-4 -z-50">
                        <div className="w-full">
                          <p>Difficulty Level {errors && (
                            <span className="text-red-1 text-sm">{errors.type}</span>
                          )}</p>
                          <select className="form w-full border bg-white p-2 text-sm rounded" value={eachQuestion.level} onChange={(e) => onLevel(indexEachQuestion, e.target.value)} size="sm" variant='outline' iconColor="blue">
                        ...
                        <div className="w-full">
                          <p>Tag</p>
                          <select className="form w-full border bg-white p-2 text-sm rounded" value={eachQuestion.tag} onChange={(e) => onTag(indexEachQuestion, e.target.value)}>
                      ...
                      /> */}
                          <QuillCreated  data={eachQuestion.question} render={renderQuestion} register={(data) => onQuestion(indexEachQuestion, data)} />
                        </div>
                      </div>
                      <div className="mt-4">
                        ...
                        <select className="form w-full border bg-white p-2 text-sm rounded" onClick={(e) => {
                          ...
                          setQuestions([...temp])
                        }} size="sm" variant='outline' iconColor="blue">
                        ...
                        <div className="mt-4">
                          ...
                          <div className="w-full  bg-white rounded-lg " style={{ lineHeight: 2 }} >
                            <QuillCreated data={eachQuestion.answer_explanation} render={renderQuestion} register={(data) => onAnswerExplanation(indexEachQuestion, data)} />
                          </div>
                        </div>
                        <Marking render={renderQuestion} mark={eachQuestion.mark} negativeMark={eachQuestion.negative_mark} setMark={(index, data) => onMark(index, data)} setNegativeMark={(index, data) => onNegativeMark(index, data)}  indexEachQuestion={indexEachQuestion} errors={errors} />
                      </div>
                    </div>
                  )
                })}
